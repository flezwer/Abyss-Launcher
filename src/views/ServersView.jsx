...
export default function ServersView({ accounts, activeAccount, settings, launching, setLaunching, setLogs, setProgress, setActiveView, notify }) {
  ...
  const connectToServer = async (srv) => {
    if (!activeAccount) { notify?.({ message: 'Añade una cuenta primero', type: 'error' }); setActiveView?.('accounts'); return }
    if (!settings) return
    ...
    if (!result.ok) { setLaunching?.(false); notify?.({ message: 'Error: ' + result.error, type: 'error' }) }
  }
    ...
    <div className="view-container">
      <div className="view-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button className="btn btn-primary btn-sm" onClick={() => setShowAdd(s => !s)}>+ Agregar</button>
        </div>
      </div>

      {showAdd && (
        <div className="card srv-add-form">
          <input autoFocus placeholder="Nombre (opcional)" value={newName} onChange={e => setNewName(e.target.value)} style={{ marginBottom: 8 }} />
          <input placeholder="IP (ej: play.hypixel.net o 1.2.3.4:25565)" value={newIp}
            onChange={e => setNewIp(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && addServer()}
          />
          <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
            <button className="btn btn-primary btn-sm" onClick={addServer}>Agregar</button>
            <button className="btn btn-ghost btn-sm" onClick={() => { setShowAdd(false); setNewName(''); setNewIp('') }}>Cancelar</button>
          </div>
        </div>
      )}
      ...
      {servers.length === 0 && !showAdd && (
        <div className="view-empty">
          ...
          <p>No hay servidores guardados</p>
          <button className="btn btn-primary btn-sm" onClick={() => setShowAdd(true)}>+ Agregar servidor</button>
        </div>
      )}
              ...
              <div className="srv-info bin-label">
                ...
                {ping && (
                  <div className="srv-ping-info">
                    {ping.online
                      ? <>
                          <span className="srv-online-txt">● Online</span>
                          {ping.ms != null && (
                      ...
                      : <span className="srv-offline-txt">● Offline</span>
                    }
                  </div>
                )}
              </div>
              <div className="srv-actions">
                <button className="btn btn-primary btn-sm srv-action-btn" onClick={() => connectToServer(srv)} disabled={launching} title="Conectar directo"><svg width="11" height="11" viewBox="0 0 24 24" fill="currentColor" stroke="none"><polygon points="5 3 19 12 5 21 5 3"/></svg></button>
                <button className="btn btn-ghost btn-sm srv-action-btn" onClick={() => pingServer(srv)} disabled={isPinging} title="Ping">
                ...
                <button className="btn btn-ghost btn-sm srv-action-btn" onClick={() => copyIp(srv)} title="Copiar IP"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>
                <button className="btn btn-ghost btn-sm srv-action-btn" onClick={() => toggleFavorite(srv.id)} title="Favorito">
